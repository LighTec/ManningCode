...
*/
    ...
    public static String approxDecrypt(String cipherTxt, String charListSortedByProbabilities) {
        CharProbability[] cipherCharListSortedByProabilities = CryptoAnalysis.sortedCharProbabilitiesOf(cipherTxt);
        int maxMatchedUpLetterCount = charListSortedByProbabilities.length();
        if (maxMatchedUpLetterCount > cipherCharListSortedByProabilities.length) {
            maxMatchedUpLetterCount = cipherCharListSortedByProabilities.length;
        }
        ...
        for (int i = 0; i < cipherTextArray.length; i++) {
            ...
            for (int j = 0; j < cipherCharListSortedByProabilities.length; j++) {
                if(cipherCharListSortedByProabilities[j].getChar() == currentCipherChar){
