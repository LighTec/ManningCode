...
public class ElevensBoard extends Board {
    ...
    private boolean containsPairSum11(List<Integer> selectedCards) {
        for (int firstCard = 0; firstCard < selectedCards.size(); firstCard++) {
            int firstCardPoints = selectedCards.get(firstCard);
            for (int secondCard = firstCard + 1; secondCard < selectedCards.size(); secondCard++) {
                if (cardAt(firstCardPoints).pointValue() + cardAt(selectedCards.get(secondCard)).pointValue() == 11) {
                    return true;
                }
            }
        }
